...
************************************************************************************** */           
	...
	function getControlled() {
		var count = api.getDeviceState(deusDevice, serviceId, "controlCount"); // PHR 02
		if (typeof(count) == "undefined") {
			count = 0;
		}
		var res = [];
		for (var i=0; i<count; i++) {
			res.push(api.getDeviceState(deusDevice, serviceId, "control"+i));
		}
		return res;
	}
	...
	function updateDeusControl(deviceId) 
	{
		...
		api.setDeviceStatePersistent(deusDevice, serviceId, "controlCount", controlled.length, 0); // PHR 01
		for (var i=0; i<controlled.length; i++) {
			api.setDeviceStatePersistent(deusDevice, serviceId, "control"+i, controlled[i], 0); // PHR 01
		}
	}
	...
	function configureDeus() 
	{
		try {
			...
			for (var j=0; j<r.length; ++j) {
				...
				for (var i=0; i<roomObj.devices.length; ++i) {
